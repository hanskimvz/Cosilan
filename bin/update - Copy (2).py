...
def argument():
    ...
    p.add_option("--normal", action="store_true", dest="normal")
...
def downloadFile(_SERVER_IP, fname):
    ...
    conn.endheaders()
    rs = conn.getresponse()
    print(rs.getheader("Content-Length"))
...
4) delete tmp_update and operate normally
    ...
    if sys.argv[0].split("/")[-1] == 'tmp_update.py' or sys.argv[0].split("/")[-1] == 'tmp_update.exe':
        if os.path.isfile("update.py"):
            fname = "../bin/update.py"
        elif os.path.isfile("update.exe"):        
            p = os.popen("taskkill /F /IM update.exe > nul")
            fname = "update.exe"
        downloadFile(_SERVER_IP, fname)
        os.system("python3 update.py --normal " + cmd_str)
        sys.exit(0)

    elif opt.normal:
        print ("delete temp file")
        if os.path.isfile("tmp_update.py"):
            os.remove(r"tmp_update.py")
        if os.path.isfile("tmp_update.exe"):
            p = os.popen("taskkill /F /IM tmp_update.exe > nul")
            os.remove(r"tmp_update.exe")

    else:
        if os.path.isfile("update.py"):
            shutil.copy2("update.py", "tmp_update.py")
            os.system("python3 tmp_update.py " + cmd_str)
        if os.path.isfile("update.exe"):
            shutil.copy2("update.exe", "tmp_update.exe")
            os.system("tmp_update.exe " + cmd_str)
    
    print ("done -- copy, download, execute new update")
