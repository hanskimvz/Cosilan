...
arrHtmlFiles = (
    ...
    "html/inc/view_device_param.php",
    "html/inc/webpage_config.php",
    "html/inc/manual_edit_db.php",
    "html/js/app.js",
...
arrBinFiles = (
    ...
    "bin/monitor.py",
    "bin/monitor.json",
    "bin/update_main.json",
    "bin/update.py",
    "bin/realtimeScreen.py",
    "bin/rtScreen.json",
...
def downAndExtract(fname, extract_path):
    global _ROOT_DIR
    os.chdir(_ROOT_DIR)
...
def makeLink(): # windows only
    ...
    with open(fname, "w") as f:
        f.write(rootdrive + "\n")
        f.write("cd \"" + _ROOT_DIR + "\\bin\"\n")
        f.write("python3.exe realtimeScreen.py\n")
...
def checkAvailabe():
    ...
    if x:
        ...
        if _WIN_GUI:
            var['txt_server_state'].set("OnLine")
        else:
            strn = "Online"
    else :
        prints("Server is Offline", "error")
        if _WIN_GUI:
    ...
    if _SERVER_MAC == mac :
        ...
        if _WIN_GUI:
            var['txt_server_state'].set("Unavailable")
        else:
            strn = "Unavailable"
        ...
        if _WIN_GUI:
            var['txt_server_state'].set("Available")
        else:
            strn = "Available"
...
def infoBox(pad=None, serverSt=""):
    ...
    label = ["txt_version_bin", "txt_version_webpage", "txt_version_param", "txt_version_code", "txt_server_state"]
...
def mysqldBox(pad=""):
    if _WIN_GUI:
        ...
        Label(pad, textvariable=var['mysql_status'], width=8).pack(side="left")
...
def startUpdate():
    if DOWN_BIN:
        prints("start downloading python binary files")
        downAndExtract("Bin-0.95-beta.zip", _ROOT_DIR + "\\BIN\\" )
